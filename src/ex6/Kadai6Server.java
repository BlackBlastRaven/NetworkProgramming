    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            while (true) {
                ...
                //ServerSocketクラスのacceptメソッドと同じように、パケットが到着するかタイムアウトなど
                //により中断されるまで動作をブロック。
                ...
                //ここから自前
                byte[] sendbuf = message.toUpperCase().getBytes();
                DatagramPacket datagramPacket = new DatagramPacket(sendbuf, sendbuf.length, packet.getSocketAddress());
                socket.send(datagramPacket);
                buf = datagramPacket.getData();
                String receivemessage = new String(buf, 0, datagramPacket.getLength());
                System.out.println("送信" + receivemessage);
